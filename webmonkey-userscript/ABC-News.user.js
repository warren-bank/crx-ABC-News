...
// @run-at       document-end
// @grant        unsafeWindow
// @homepage     https://github.com/warren-bank/crx-ABC-News/tree/webmonkey-userscript/es5
// @supportURL   https://github.com/warren-bank/crx-ABC-News/issues
// @downloadURL  https://github.com/warren-bank/crx-ABC-News/raw/webmonkey-userscript/es5/webmonkey-userscript/ABC-News.user.js
// @updateURL    https://github.com/warren-bank/crx-ABC-News/raw/webmonkey-userscript/es5/webmonkey-userscript/ABC-News.user.js
...
var extract_videos = function(all_data) {
  var bands, band, block, video
  var videos = []

  try {
    bands = all_data.page.content.section.bands

    if (Array.isArray(bands) && bands.length) {
      for (var i1=0; i1 < bands.length; i1++) {
        band = bands[i1]

        if ((band instanceof Object) && Array.isArray(band.blocks) && band.blocks.length) {
          for (var i2=0; i2 < band.blocks.length; i2++) {
            block = band.blocks[i2]

            if ((block instanceof Object) && (block.componentKey === 'liveVideoCarousel') && (block.items instanceof Object) && Array.isArray(block.items.playlist) && block.items.playlist.length) {
              for (var i3=0; i3 < block.items.playlist.length; i3++) {
                video = block.items.playlist[i3]

                if ((video instanceof Object) && (video.videos instanceof Object) && video.title && video.videos.id) {
                  videos.push({
                    id:          video.videos.id,
                    title:       video.title,
...
var download_video_url = function(video_index, callback) {
  ...
  url     = 'https://abcnews.go.com/video/itemfeed?id=' + video.id
  headers = null
  data    = null
  video   = null
  ...
  xhr_callback = function(json) {
    ...
    var media_filter = function(media_item) {
      return (
           (media_item instanceof Object)
        && (media_item.temporalType === 'live')
        && (media_item["@attributes"] instanceof Object)
        && (media_item["@attributes"]["isLive"] === 'true')
        && (media_item["@attributes"]["medium"] === 'video')
        && (media_item["@attributes"]["type"])
        && (media_item["@attributes"]["url"])
      )
    }

    var media_map = function(media_item) {
      return {
        video_type: media_item["@attributes"]["type"].toLowerCase(),
        video_url:  media_item["@attributes"]["url"]
      }
    }

    // most  preferred format has highest index
    // least preferred format has  0 index
    // unrecognized    format has -1 index
    var media_sort_order = ['application/x-mpegurl', 'application/dash+xml', 'video/mp4', 'video/f4m'].reverse()

    var media_sort = function(media_item_1, media_item_2) {
      var t1 = media_item_1.video_type
      var t2 = media_item_2.video_type

      var i1 = media_sort_order.indexOf(t1)
      var i2 = media_sort_order.indexOf(t2)

      // order from most preferred format to least
      return (i1 > i2)
        ? -1
        : (i1 < i2)
          ? 1
    ...
    try {
      media = json["channel"]["item"]["media-group"]["media-content"]
      media = media.filter(media_filter)
      media = media.map(media_map)
      media = media.sort(media_sort)
...
var process_video = function(video_index, callback) {
  ...
  download_video_url(video_index, callback)
}
...
var reinitialize_dom = function() {
  ...
  var html = {
    ...
    "body": [
      '<div class="' + constants.dom_classes.div_show     + '"><h2><a target="_blank" href="https://abcnews.go.com/live">ABC News: Live Streams</a></h2></div>',
...
var init = function() {
  ...
  if (unsafeWindow.window['__abcnews__'] instanceof Object)
    data = unsafeWindow.window['__abcnews__']
